'use client';
import { AppButton, AppTextInput, BottomSheet, Navbar, Radial, Row, TextP } from '@/components';
        ...
        <form className="w-full px-5 py-4 flex flex-col items-center justify-center" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="w-full grid grid-cols-4 gap-2 my-4">
            {AirtimeData['Nigeria'].amount.map((val, i) => (
              ...
              >
                <TextP className="text-primary text-[11px] font-semibold">{`${AirtimeData['Nigeria'].symbol}  ${val.toString()}`}</TextP>
              </div>
            ))}
          </div>
